...
{
#if 0
#define ZEO_TRI3D_DUALXFORM_MIN_OFFSET ( 1.0e-6 )
  double d;
  ...
  return zIsTiny( ( d = zVec3DInnerProd(zTri3DNorm(t),p) ) ) ? NULL : zVec3DDiv( zTri3DNorm(t), d, dx );
#endif
}
  ...
  if( ib ){
    for( i=0; i<zPH3DFaceNum(ph1); i++ )
      if( zColChkTriAABox3D( ( tri = zPH3DFace(ph1,i) ), ib ) ){
        if( !_zTri3DDualXform_a( tri, &p1, &p_temp ) || !zVec3DDataAdd( &data1, &p_temp ) ){
          phcol = NULL;
          goto TERMINATE;
        }
        n++;
