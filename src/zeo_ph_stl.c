...
static zVec3D *_zPH3DFReadSTL_BinVec(FILE *fp, zVec3D *v)
{
  ...
  if( fread( &val1, sizeof(float), 1, fp ) < 1 ) goto ZPH3DFREADSTL_ERROR;
  if( fread( &val2, sizeof(float), 1, fp ) < 1 ) goto ZPH3DFREADSTL_ERROR;
  if( fread( &val3, sizeof(float), 1, fp ) < 1 ) goto ZPH3DFREADSTL_ERROR;
